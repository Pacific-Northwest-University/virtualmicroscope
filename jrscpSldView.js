...
function sldInitializeView() {
	...
	document.getElementById("sldBndBox").onwheel.bubbles = true;
		...
		//	1/02/20: added useCaptuer="true" => this should have made event.target = "sldBndBox"
...
function sldUnWait() {
	...
	document.getElementById("sldBndBox").ondblclick = function(event){sldMusDblClk(event)};
	sldResetWaitCurs();
		// if 2-fingers are on screen:
		//	(1) turn-off "wait"-clock icon.
	...
	tchWaitMenuIconOff();  // turn-off menu "wait" clock icons if they were on.
	return;
	}
...
function sldResetWaitCurs() {
	var i;
	for (i = 0; i < glbDivWaitArr.length; i++) {
