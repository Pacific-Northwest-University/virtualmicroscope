...
function sldUpdtFtoSV(curZ) {
	...
	//	sldRestrictFZ(isMusMv) removes from sldVw[] all planes not protected by 
...
function sldRestrictFZ(isMusMv, limCurZ) {
	...
	for (indZ = 0; indZ < dbMaxZ; indZ++) {
		...
		if (indZ < glbSldZFLim) {  // if Z < glbSldZFLim => only F == glbSldFDef allowed
					// if within Z-buffer range; preserve F =- sldDefault
